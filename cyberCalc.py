...
deb = False
while not deb:
    for i in range(len(courses)): print(courses[i].name)
    complete = input(DECOR + "Enter the latest course you have fully completed\nIf you haven't yet completed a course, enter \"nothing\" without the quotation marks:  ")
    if complete.upper() == "nothing".upper():break
    for i in range(len(courses)):
        if courses[i].name.upper() == complete.upper():
...
while True:
    nextCourseCompleted = int(input(DECOR + "How many assignments have you completed in " + courses[startIndex].name + " (The next course)? Zero is a Valid answer!\nEnter: "))
...
while not deb:
    for i in range(len(courses)): print(courses[i].name)
    complete = input(DECOR + "Enter the course you want to have done by the end of the alloted time: ")
    for i in range(len(courses)):
        if courses[i].name.upper() == complete.upper():
            endIndex = i+1
            deb = True
